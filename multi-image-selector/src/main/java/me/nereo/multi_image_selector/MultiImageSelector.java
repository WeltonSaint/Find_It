...
public class MultiImageSelector {
    ...
    public MultiImageSelector showCamera(boolean show){
        sSelector.mShowCamera = show;
        return sSelector;
    }

    public MultiImageSelector count(int count){
        sSelector.mMaxCount = count;
        return sSelector;
    }

    public MultiImageSelector single(){
        sSelector.mMode = MultiImageSelectorActivity.MODE_SINGLE;
        return sSelector;
    }

    public MultiImageSelector multi(){
        sSelector.mMode = MultiImageSelectorActivity.MODE_MULTI;
        return sSelector;
    }

    public MultiImageSelector origin(ArrayList<String> images){
        sSelector.mOriginData = images;
        return sSelector;
    }
    ...
    private Intent createIntent(Context context){
        ...
        if(sSelector.mOriginData != null){
            intent.putStringArrayListExtra(MultiImageSelectorActivity.EXTRA_DEFAULT_SELECTED_LIST, sSelector.mOriginData);
        }
        intent.putExtra(MultiImageSelectorActivity.EXTRA_SELECT_MODE, sSelector.mMode);
        return intent;
    }
}
